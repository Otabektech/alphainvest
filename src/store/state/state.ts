...
export const initialState = {
  ...
  ipoCalendar: {
    data: { weekly: {}, monthly: {} },
    currentWeek: {
      from: moment().year(2022).week(1).startOf("isoWeek").format("YYYY-MM-DD"),
      to: moment().year(2022).week(1).endOf("isoWeek").format("YYYY-MM-DD"),
    },
    currentMonth: {
      from: moment().year(2022).month(0).startOf("month").format("YYYY-MM-DD"),
