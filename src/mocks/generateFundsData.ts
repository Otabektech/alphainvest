...
const generateFundsData = (FUNDS: string[], COMPANIES: Companies) => {
  ...
  for (let idx = 0; idx < FUNDS.length; idx++) {
    ...
    for (let i = startIdx; i < endIdx && i < companies.length; i++) {
      ...
      const shareOfTheRound = randomShares[pickRandomIdx(randomShares.length)];
      ...
      const ownershipPercentage =
        (shareOfTheRound * companyFundingRounds[investedRoundIdx].givenEquity) /
        100;

      const impliedValue =
        (ownershipPercentage * company.business.potentialExitValuation) / 100;
